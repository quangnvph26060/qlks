...
@push('script')
    <script>
        $(document).ready(function() {
            var formEconomyEdit = {
                'code': { // passwword thì nên đặt là name trong input đó
                    'element': document.getElementById('code'), // id trong input đó
                    ...
                    'validations': [{
                        ...
                        //     },
                        //     'message': generateErrorMessage('INHOA', 'Mã kho')
                        // },
                    ]
                },
            ...
            $('#btn-add-hotel').on('click', function() {
                ...
                row += `
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form id="btn-submit-hotel" action="{{ route('admin.warehouse.add.warehouse') }}"
                                    method="POST">
                                    @csrf
                                    <!-- Input 1 -->
                                    <div class="mb-3">
                                    ...
                                    <!-- Input 2 -->
                                    <div class="mb-3">
                                        <label for="hotelLocation" class="form-label">Tên kho</label>
                ...
                formEconomyEdit.code.element = document.getElementById('code');
                formEconomyEdit.code.error = document.getElementById('ma_coso_error');
                formEconomyEdit.name.element = document.getElementById('name');
                formEconomyEdit.name.error = document.getElementById('ten_coso_error');
            });
            // sửa
            $('.btn-edit-hotel').on('click', function() {
                ...
                // ajax request
                $.ajax({
                    url: `{{ route('admin.warehouse.edit.warehouse', '') }}/${dataId}`,
                    type: 'POST',
                    success: function(data) {
                        if (data.status == 'success') {
                            ...
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h1 class="modal-title fs-5" id="exampleModalLabel">Chỉnh sửa kho</h1>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    <form id="btn-submit-hotel-update" action="{{ route('admin.warehouse.update.warehouse', '') }}/${data.data['id']}"
                                        method="POST">
                                        @csrf
                                        <!-- Input 1 -->
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control " name="code" id="code"
                                                placeholder="Nhập mã cơ sở" value="${data.data['code']}">
                                            <span class="invalid-feedback d-block" style="font-weight: 500"
                                                id="ma_coso_error"></span>
                                        </div>
                                        <!-- Input 2 -->
                                        <div class="mb-3">
                                            <label for="hotelLocation" class="form-label">Tên kho</label>
                                            <input type="text" class="form-control " name="name" id="name"
                                                placeholder="Nhập tên khách sạn" value="${data.data['name']}">
                                            <span class="invalid-feedback d-block" style="font-weight: 500"
                                                id="ten_coso_error"></span>
                                        </div>
                                        <div class="mb-3">
                            ...
                            formEconomyEdit.code.element = document.getElementById(
                                'code');
                            formEconomyEdit.code.error = document.getElementById(
                                'ma_coso_error');
                            formEconomyEdit.name.element = document.getElementById(
                                'name');
                            formEconomyEdit.name.error = document.getElementById(
                                'ten_coso_error');
                        }
                    },
            ...
            $('.icon-delete-room').on('click', function() {
                ...
                Swal.fire({
                    ...
                    text: 'Bạn có chắc chắn muốn xóa kho này không?',
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        // ajax
                        $.ajax({
                            url: `{{ route('admin.warehouse.delete.warehouse', '') }}/${dataId}`,
                            type: 'POST',
                            success: function(data) {
                                if (data.status === 'success') {
                                    rowToDelete.remove();
