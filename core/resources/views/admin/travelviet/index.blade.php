    ...
    (function(){
        ...
        function render(data){
            ...
            if (currentKind === 'roomtypes') {
                ...
                } else if (data && data.data && data.data.list_cate_room && Array.isArray(data.data.list_cate_room)) {
                ...
                bodyRows = list.map(item => {
                    ...
                    const cells = Object.keys(importantFields).map(key => {
                        ...
                        if (key === 'room_id') {
                            ...
                            value = item.price_room ?? '';
                        } else if (key === 'room_image') {
                            // Xử lý hiển thị hình ảnh
                            const imageUrl = item.room_image ?? '';
                            if (imageUrl && imageUrl.trim() !== '') {
                                value = `<img src="${escapeHtml(imageUrl)}" alt="Room Image" style="width: 50px; height: 50px; object-fit: cover; border-radius: 4px;" onerror="this.style.display='none'">`;
                            } else {
        ...
        pSaveBtn.addEventListener('click', function(){
            alert('chức năng đang triển khai');
            ...
            if (!confirm(`Bạn có chắc chắn muốn lưu ${selected.length} ${itemType} vào database?\n\n${itemType.charAt(0).toUpperCase() + itemType.slice(1)}: ${itemNames}`)) {
                return;
            }
            ...
            $.ajax({
                ...
                if (currentKind === 'roomtypes') itemType = 'loại phòng';
                if (currentKind === 'rooms') itemType = 'phòng';
                alert(`✅ Đã lưu thành công ${res?.saved ?? selected.length} ${itemType} vào database!`);
                // Reset selection
