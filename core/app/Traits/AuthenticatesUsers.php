...
trait AuthenticatesUsers
{
    ...
    protected function canLoginBySubdomain(Request $request)
    {
        ...
        $usernamePart = strstr($username, '@', true) ?: $username;
        // dd($subdomain == $usernamePart);
        return $subdomain == $usernamePart;
    }

    protected function getSubdomainFromHost($host)
    {
        $baseDomain = config('domain.base_domain');
        // Kiểm tra host có kết thúc bằng baseDomain không
        if (str_ends_with($host, $baseDomain)) {
            // Lấy phần trước baseDomain
            $subdomain = substr($host, 0, -strlen($baseDomain));
            // Bỏ dấu chấm cuối nếu có
            $subdomain = rtrim($subdomain, '.');

            // Nếu có subdomain thì trả về, không thì trả null
            return $subdomain !== '' ? $subdomain : null;
        }

        // Không phải domain dạng *.fasthotels.vn thì abort
        abort(403, 'Sai đường dẫn');
    }
