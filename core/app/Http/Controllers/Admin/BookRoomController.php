...
class BookRoomController extends Controller
{
        ...
        try {
            ...
            foreach ($request->room as $index => $item) {
                ...
                if (!empty($room['bookingId'])) {
                    ...
                    //     'room_price'      => DB::raw('room_price + ' . $roomPice),
            ...
            foreach ($request->room as $index => $item) {
                ...
                if (!empty($room['bookingId'])) {
                    ...
                    if ($check_in) {
                        ...
                        $request->method == 'check_in'
                            ? bookingActionRecord($check_in->id, $admin_id, $room['room'], 'Sửa nhận phòng', 'check_in')
                            : bookingActionRecord($check_in->id, $admin_id, $room['room'], 'Sửa đặt phòng', 'room_booking');
                    ...
                    $request->method == 'check_in'
                        ...
                        : bookingActionRecord($check_in_new->id, $admin_id, $room['room'], 'Đặt phòng', 'room_booking');
                    saveRoomStatusHistory($room['room'], $room['dateIn'], $room['dateOut'], 2);
                    // TỔNG GIẢM GIÁ
                    $totalDiscount += $discountAmount;
                    // TỔNG ĐẶT CỌC 
        ...
        try {
            return DB::transaction(function () use ($ids, $method) {
                ...
                foreach ($checkIns as $checkIn) {
                    ...
                    bookingActionRecord($checkIn->id, $admin_id, $checkIn->room_code, 'Xoá nhận phòng', 'check_in');
                    $checkIn->delete();
                }
    ...
    public function checkInEdit(Request $request, $id)
    {
        ...
        $roomBookings = $roomBookings->get();
        $receipt = ReceiptAndPayment::with('paymentTransactions')
            ->where('checkin_id', $id)
            ->first();

        $totalPayment = $receipt
            ? $receipt->paymentTransactions->sum('amount')
            : 0;
    ...
    public function findRoomBookingId(Request $request)
    {
        ...
        foreach ($request->booking_id as $item) {
            ...
            $result = !empty($item['method']) ? $result->get() : $result->first();
            if (!$result) {
                $result  = RoomBooking::query()
                    // ->active()
                    ->where('status', Status::DISABLE)
            ...
            if (!empty($item['method'])) {
                // $result là collection
                foreach ($result as $itemResult) {
                    $roomBooking[] = $itemResult->toArray();
                }
            } else {
                // $result là một object
                if ($result) {
        ...
        foreach ($roomBooking as $roomBook) {
            ...
            $data[$bookingId]['rooms'][] = [
                ...
                'room_type'     => $roomBook['room_change_info'] ? $roomBook['room_change_info']['room']['room_type']['id'] : $roomBook['room']['room_type']['id'],
