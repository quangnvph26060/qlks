...
class DepositController extends Controller
{
    ...
    public function reject(Request $request)
    {
        ...
        $deposit = Deposit::where('id', $request->id)->where('status', Status::PAYMENT_PENDING)->with('user')->firstOrFail();
        ...
        //action log
        bookingActionRecord($booking->id, auth()->guard('admin')->id(), 0, 'payment_reject');

        notify($deposit->user, 'PAYMENT_MANUAL_REJECT', [
