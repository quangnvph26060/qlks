                ...
                Object.values(groupedRooms).forEach(group => {
                    ...
                    group.rooms.forEach(item => {
                        ...
                        ${status_code == 2 && checkTime
                            ...
                            <div class="room-info    ${status_code == 2 && checkTime
                                ? 'text-white'
                                : status_code == 3 && checkTime
                                    ? 'text-white'
...
function initViewScriptGird() {
    ...
    $(document).ready(function () {
                        ...
                        $('#myModal-booking-edit').modal('show').on('shown.bs.modal', function () {
                            ...
                            response.data.forEach(item => {
                                ...
                                item.room_bookings.forEach((room, index) => {
                                    ...
                                    var tr = `
                                        <tr data-room-id="${room.room_id}" data-price="${room.total_amount}" data-status="${room.status}"
        ...
        $(document).off("click", ".check_in_now").on("click", ".check_in_now", function (e) {
            e.stopPropagation();
            let roomId = $(this).attr("data-room-id");
            let roomType = $(this).attr("data-room-type-id");
            $(".btn-book").attr("data-method", "check_in");
            let data = {
                room: roomId,
                room_type: roomType,
                date: formattedDates,
